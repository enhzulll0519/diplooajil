          ...
          builder: (context, setState) {
            return Container(
              ...
              child: Column(
                ...
                children: [
                  Text("Settings", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.blueAccent)),
                  Divider(color: Colors.blueAccent, thickness: 1.5), // Stylish divider
                  ...
                  // Game time selection
                  ListTile(
                    title: Text("Game Time", style: TextStyle(fontSize: 18, color: Colors.black)),
                    trailing: DropdownButton<int>(
                      value: _selectedTime,
                      items: [30, 60, 90].map((int time) {
                        return DropdownMenuItem(value: time, child: Text("$time sec"));
                      }).toList(),
                      onChanged: (value) {
                        setState(() {
                          _selectedTime = value!;
                  ...
                  // Font size selection
                  ListTile(
                    title: Text("Font Size", style: TextStyle(fontSize: 18, color: Colors.black)),
                    trailing: DropdownButton<double>(
                      value: _selectedFontSize,
                      items: [16.0, 18.0, 20.0, 24.0].map((double size) {
                        return DropdownMenuItem(value: size, child: Text("$size px"));
                      }).toList(),
                      onChanged: (value) {
                        setState(() {
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.blueAccent, // Stylish button color
                      minimumSize: Size(double.infinity, 50),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                      shadowColor: Colors.blueAccent.withOpacity(0.4), // Add shadow for depth
                      elevation: 6, // Elevation for button
                    ),
                    child: Text("Save & Close", style: TextStyle(color: Colors.white, fontSize: 18)),
                  ),
                ],
              ),
            );
    ...
    return Column(
      children: [
        ElevatedButton(
          ...
          onPressed: () {},
          child: Text('Play Again', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold,)),
        ),
        SizedBox(height: 10),
      ],
    );
